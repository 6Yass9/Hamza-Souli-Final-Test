...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  const APPOINTMENT_TYPES = ['Wedding', 'Wteya', 'Henna', 'Reception', 'Consultation', 'Other'] as const;

  // Client comments (stored locally in browser to avoid DB changes)
  const [clientComments, setClientComments] = useState<Record<string, string>>(() => {
    try {
      const raw = localStorage.getItem('client_comments_v1');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('client_comments_v1', JSON.stringify(clientComments));
    } catch {}
  }, [clientComments]);
  ...
  const handleCreateAdminAppointment = async (e: React.FormEvent) => {
    ...
    const client = clients.find(c => c.id === newAppointmentData.clientId);
    if (!client) {
      alert('Veuillez sélectionner un client.');
      return;
    }
    if (!client.phone) {
      alert('Ce client n\'a pas de numéro de téléphone. Ajoutez-le dans le dossier client.');
      return;
    }
    ...
    if (!created) {
      alert('Échec de création du rendez-vous.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex relative">
      ...
      <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-64'} bg-stone-900 text-stone-400 flex flex-col fixed h-full z-10 overflow-y-auto transition-all duration-200`}>
        ...
        <nav className="flex-1 px-4 space-y-2">
          <button
            ...
            className={`w-full px-4 py-3 rounded flex items-center gap-3 transition-colors ${isSidebarCollapsed ? 'justify-center' : 'text-left'} ${
              view === 'overview' ? 'bg-stone-800 text-white' : 'hover:bg-stone-800'
            }`}
          >
            <CalendarIcon size={18} /><span className={isSidebarCollapsed ? 'hidden' : ''}>{t('admin.sidebar.overview')}</span>
          </button>
          ...
          >
            <CalendarIcon size={18} /><span className={isSidebarCollapsed ? 'hidden' : ''}>{t('admin.sidebar.calendar') || 'Calendrier'}</span>
          </button>
          ...
          >
            <Users size={18} /><span className={isSidebarCollapsed ? 'hidden' : ''}>{t('admin.sidebar.clients')}</span>
          </button>
          ...
          >
            <Image size={18} /><span className={isSidebarCollapsed ? 'hidden' : ''}>{t('admin.sidebar.gallery')}</span>
          </button>
          ...
          >
            <Shield size={18} /><span className={isSidebarCollapsed ? 'hidden' : ''}>{t('admin.sidebar.staff') || 'Staff'}</span>
          </button>
        </nav>
            ...
            <header className="flex justify-between items-center mb-8">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="bg-stone-900 text-white px-5 py-2 text-xs uppercase tracking-widest hover:bg-stone-700 transition-colors flex items-center gap-2"
                >
                  <Plus size={14} /> {t('admin.overview.newAppointment') || 'Nouveau rendez-vous'}
                </button>
              </div>
            </header>
          ...
          <div className="bg-white p-8 rounded-lg shadow-2xl max-w-md w-full animate-fade-in" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateAppointment} className="space-y-6">
              ...
              <div className="space-y-1">
                ...
                <select
                  value={editingAppointment.type || 'Other'}
      ...
      {isCreatingAppointment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setIsCreatingAppointment(false)}>
          <div className="bg-white p-8 rounded-lg shadow-2xl max-w-lg w-full animate-fade-in" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-serif text-2xl text-stone-900">{t('admin.overview.newAppointment') || 'Nouveau rendez-vous'}</h3>
              <button onClick={() => setIsCreatingAppointment(false)} className="text-stone-400 hover:text-stone-800">
                <X size={24} />
              </button>
            </div>

            <form onSubmit={handleCreateAdminAppointment} className="space-y-6">
              <div className="space-y-1">
                <label className="text-xs uppercase tracking-wide text-stone-500">{t('admin.overview.form.client') || 'Client'}</label>
                <select
                  value={newAppointmentData.clientId}
                  onChange={e => setNewAppointmentData({ ...newAppointmentData, clientId: e.target.value })}
                  className="w-full border-b border-stone-300 py-2 focus:outline-none focus:border-stone-800 bg-transparent text-stone-800"
                  required
                >
                ...
                <p className="text-[11px] text-stone-400">{t('admin.overview.form.clientHint') || 'Le client doit avoir un numéro de téléphone pour voir ses rendez-vous.'}</p>
              </div>

              <div className="space-y-1">
                <label className="text-xs uppercase tracking-wide text-stone-500">{t('admin.overview.form.assignedStaff') || 'Staff assigné'}</label>
                <select
                  value={newAppointmentData.staffId}
                  onChange={e => setNewAppointmentData({ ...newAppointmentData, staffId: e.target.value })}
                  className="w-full border-b border-stone-300 py-2 focus:outline-none focus:border-stone-800 bg-transparent text-stone-800"
                >
                  <option value="">{t('admin.overview.unassigned') || 'Non assigné'}</option>
                  {staff.map(s => (
                    <option key={s.id} value={s.id}>
                      {s.name}{s.email ? ` (${s.email})` : ''}
                    </option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs uppercase tracking-wide text-stone-500">{t('admin.overview.form.date') || 'Date'}</label>
                  <input
                    type="date"
                ...
                <div className="space-y-1">
                  <label className="text-xs uppercase tracking-wide text-stone-500">{t('admin.overview.form.time') || 'Heure'}</label>
                  <input
                    type="time"
              ...
              <div className="space-y-1">
                <label className="text-xs uppercase tracking-wide text-stone-500">{t('admin.overview.form.type') || 'Type'}</label>
                <select
              ...
              <div className="space-y-1">
                <label className="text-xs uppercase tracking-wide text-stone-500">{t('admin.overview.form.status') || 'Statut'}</label>
                <select
                  value={newAppointmentData.status}
                  onChange={e => setNewAppointmentData({ ...newAppointmentData, status: e.target.value as any })}
                  className="w-full border-b border-stone-300 py-2 focus:outline-none focus:border-stone-800 bg-transparent text-stone-800"
                >
                  <option value="pending">{t('admin.overview.status.pending') || 'En attente'}</option>
                  <option value="confirmed">{t('admin.overview.status.confirmed') || 'Confirmé'}</option>
          ...
          <div className="bg-white rounded-lg shadow-2xl max-w-4xl w-full h-[90vh] flex flex-col animate-fade-in overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-stone-50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 rounded shadow-sm border border-stone-200">
                    <h3 className="font-serif text-xl mb-4 text-stone-800 flex items-center gap-2">
                      <FileText size={18} /> {t('admin.clients.comments') || 'Commentaires'}
                    </h3>
                    <textarea
                      value={clientComments[selectedClient.id] || ''}
                      onChange={(e) => setClientComments({ ...clientComments, [selectedClient.id]: e.target.value })}
                      placeholder={t('admin.clients.commentsPlaceholder') || 'Ajouter des notes internes sur ce client (visible uniquement pour l’admin)...'}
                      className="w-full border border-stone-200 p-3 text-sm text-stone-700 h-32 resize-none focus:outline-none focus:border-stone-400 bg-white"
                    />
                    <p className="text-[11px] text-stone-400 mt-2">
                      {t('admin.clients.commentsHint') || 'Ces commentaires sont sauvegardés dans ce navigateur (pas encore dans la base de données).'}
                    </p>
                  </div>
