...
export const ClientDashboard: React.FC<ClientDashboardProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // appointments for this client (confirmed first)
      const apps = await api.getClientAppointments({ name: user.name, phone: user.phone });
      const confirmed = apps.filter(a => a.status === 'confirmed' || a.status === 'pending');
      setAppointments(confirmed.slice(0, 1));
